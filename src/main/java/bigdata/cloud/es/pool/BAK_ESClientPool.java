...
public class BAK_ESClientPool {
	 ...
	 * @return
	 */
	public static BAK_ESClientPool getInstance(){
		if(esClientPool == null){
			esClientPool = new BAK_ESClientPool();
		}
		return esClientPool;
	}
	...
	private static void makeClients(String es_hosts, int es_port, String es_cluster_name, int es_client_pool_size) {
		...
		for (int i = 0; i < es_client_pool_size; i++) {
			futures.add(executorService.submit(new BAK_ESClientMakerThread(settings, addressArray)));
		}
		try {
			for (int i = 0; i < futures.size(); i++) {
				esClientQueue.offer(futures.get(i).get());
			}
		} catch (InterruptedException e) {
			e.printStackTrace();
